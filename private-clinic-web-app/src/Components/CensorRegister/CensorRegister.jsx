...
export default function CencorRegister() {
  ...
  useEffect(() => {
    if (currentUser !== null) {
      ...
      if (urs !== null && urs.statusIsApproved && urs.statusIsApproved.status === "PAYMENTPHASE2") {
        getMEByMrlId(urs.id);
      }
    }
                ...
                allRegisterScheduleList.content.map((mrl, index) => {
                  return (
                      <tr key={index} className="align-middle">
                        ...
                        <td>{mrl.name}</td>
                        <td>{mrl.favor}</td>
                        <td>{mrl.statusIsApproved && mrl.statusIsApproved.note ? mrl.statusIsApproved.note : ""}</td>
                        <td>
                        ...
                        <td className="align-middle text-center">
                          {getStatusClass(
                              mrl.statusIsApproved && mrl.statusIsApproved.status ? mrl.statusIsApproved.status : ""
                          )}
                        </td>
                        <td className="align-middle">
                          {mrl.statusIsApproved && mrl.statusIsApproved.status ? mrl.statusIsApproved.status : "Chưa xác định"}
                        </td>
                        <td>
                          {(mrl.statusIsApproved && (mrl.statusIsApproved.status === "PAYMENTPHASE1" ||
